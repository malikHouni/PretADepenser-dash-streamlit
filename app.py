...
def showDashData(choosen_client):
    if choosen_client == "" :
        ...
        if (df.loc[currUser]['CODE_GENDER'].item() == "F"):
            currImage="F"
        else:
            currImage="M"
        currentStr = f'''
        ...
        components.html(currentStr)
        if btnProfil:
            ProfilStr =f'''<h3>Your profil:</h3><li>name: </li><li>firstname: </li><li>sex:{df.loc[currUser]['CODE_GENDER'].item()}</li><li>Loan Type:{df.loc[currUser]['NAME_CONTRACT_TYPE'].item()} </li>'''
            components.html(ProfilStr)
        if btnLoan:
            if df.loc[currUser]['TARGET'].item() >0.6:
                refundLoanVal="Good"
